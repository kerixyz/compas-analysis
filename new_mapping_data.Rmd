...
faces$cat <-
    cut(faces$age, 
    breaks = c(0, 21, 35, 45, 55, Inf), 
    labels = c('1', '2', '3', '4', '5'))

faces$sex <- 
    case_when(faces$sex == 'F' ~ 1, faces$sex == 'M' ~ 0)

faces$race <- 
    case_when(faces$race == 'A' ~ 4, faces$race == 'B' ~ 2, faces$race == 'L' ~ 3, faces$race == 'W' ~ 1)
...
df_map <- 
  left_join(def2, faces2, by = c('features', 'index'))
